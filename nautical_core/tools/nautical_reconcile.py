#!/usr/bin/env python3
"""Repair Nautical chains completed by hookless clients."""
...
def _candidate_rows(task_bin: str) -> list[dict[str, Any]]:
    rows = _export(task_bin, ["status:completed", "chain:on", "chainID.not:"])
    return [row for row in rows if reconcile.is_orphan_completion_candidate(row)]
...
def _existing_children(task_bin: str, parent: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not chain_id:
        return []
    return _export(task_bin, [f"chainID:{chain_id}", f"link:{next_link}", "status.not:deleted"], timeout=30.0)

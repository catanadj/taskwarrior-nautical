...
"""Heavy on-add implementation loaded lazily by the executable wrapper.
    ...
    if cpmax and cpmax > 0:
        ...
        for _ in range(steps):
            if tokens:
                fmax = _cp_add_td(
                    fmax,
                    _cp_sequence_period_for_link(tokens, cp_str, link_no, chain_id),
                )
                link_no += 1
            else:
...
def _handle_cp_preview_on_add(
    ...
    *,
    ...
    td = _cp_sequence_period_for_link(tokens, cp_str, 1, chain_id)
    ...
    if is_dynamic:
        exact_until_count, final_until_dt = _cp_sequence_until_summary(
            due_dt,
            until_dt,
            tokens,
            cp_str,
    ...
    if is_dynamic:
        ...
        preview = _cp_sequence_preview_lines(
            due_dt,
            until_dt,
            limit,
            tokens,
    ...
    try:
        ...
        if exhausted_type is not None and isinstance(exc, exhausted_type):
            if exc.is_date_limit:
                scheduler_message = core._import_sibling("scheduler_models").occurrence_exhaustion_message(exc)
            else:

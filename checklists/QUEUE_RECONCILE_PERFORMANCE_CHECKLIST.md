...
input paths. Configured full-tree strict mypy was initially noisy under the
legacy opt-in error codes, but the configured gate now passes all 173 source
files. The second Termux device was unavailable, so the two-device gate also
remains open.
...
Installed-layout verification (2026-08-25): the disposable real-Taskwarrior
black-box run passed CP completion, interrupted child import and retry,
parent-link recovery, deletion handling, Doctor, integrity, reconcile dry-run
and apply, queue status, and idempotent replay. The temporary Taskdata was
kept for post-run inspection at `/tmp/nautical-black-box-uyfxovc3/taskdata`.

The follow-up multi-hop extension then forced two expired hookless completions
and recovered links 3 and 4 successfully.

Mypy follow-up (2026-08-25): configured full-tree mypy now passes all 173
source files, and the strict targeted lifecycle set passes all 17 modules.
...
- [x] Run `py_compile`, configured full-tree and strict targeted mypy, golden
  tests, deterministic shuffled golden tests, black-box tests, deployment
  sanity, stress, soak, and compatibility checks. The configured mypy gate
  passes all 173 source files; a dedicated real-installed installer smoke run
  is covered by the staged black-box installation verification.
...
- [x] Run a staged installed-layout recovery test with real Taskwarrior: queue
  completion, interrupted child import, parent-link recovery, reconcile dry
  run, reconcile apply, and idempotent replay.
- [x] Exercise multi-hop expiration explicitly in the staged installed layout;
  the black-box recovery run recovered successive links 3 and 4.

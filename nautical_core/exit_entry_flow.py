...
def precheck_parent_guard(
    ...
    *,
    ...
    if isinstance(parent_res, Unavailable):
        return "break" if services.retry_or_manual_review_for_lock(ctx.entry, ctx.idx, ctx.state) else "continue"
    parent = parent_res.value if isinstance(parent_res, Found) else None
    if not isinstance(parent, dict):
        ctx.state.manual_review(ctx.entry, "stale spawn intent: parent missing")
    ...
    ctx.state.manual_review(ctx.entry, f"stale spawn intent: {mismatch}")
    ctx.state.reset_lock_streak()
    return "continue"
...
def precheck_parent_link_state(
    ...
    *,
    ...
    if state_res.state == "locked":
        return ("break", False) if services.retry_or_manual_review_for_lock(ctx.entry, ctx.idx, ctx.state) else ("continue", False)
    if state_res.state in {"conflict", "missing", "invalid"}:
        ctx.state.manual_review(ctx.entry, f"parent update failed: {state_res.err}")
        ctx.state.reset_lock_streak()
    ...
    if not isinstance(export_res, Found):
        if isinstance(export_res, Unavailable):
            if ctx.spawn_intent_id:
                services.diag(f"task lock active; retry deferred (intent={ctx.spawn_intent_id})")
            return ("break", False) if services.retry_or_manual_review_for_lock(ctx.entry, ctx.idx, ctx.state) else ("continue", False)
        ...
        if not import_res.ok:
            ...
            if not isinstance(services.export_uuid(ctx.child_uuid, prefer_cache=False), Found):
                ...
                ctx.state.manual_review(ctx.entry, failure_reason)
                ctx.state.reset_lock_streak()
                return "continue", False
        imported = True
...
def apply_parent_update_for_entry(
    ...
    *,
    ...
    if imported:
        services.cleanup_orphan_child(ctx.child_uuid, ctx.spawn_intent_id)
    ctx.state.manual_review(ctx.entry, f"parent update failed: {update_res.err}")
    ctx.state.reset_lock_streak()
    return "continue"

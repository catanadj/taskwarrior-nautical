...
- [x] Keep focused tests for fast diagnosis; do not replace full discovery with
  an allowlist of modules.
- [x] Document the distinction between the unit suite and the custom golden
  suite in the verification instructions.
- [x] Confirm the CI command discovers the same unit count as the local command
  for the same revision and environment. Both use the canonical command below;
  local discovery currently passes 328 tests.
...
- [x] Intermediate branch commits may remain uninstalled; use only disposable
  runtime state until final wiring and verification.
- [x] Do not combine correctness repairs and the first optimization in one
...
- [x] Consider scheduler session binding or compiled recurrence helpers only
  after request-shaped collection is measured and accepted; require parity
  tests before replacing dynamic scheduler behavior. Deferred by the explicit
  decision to remove unused synchronous hint precomputation; it is future work,
  not a prerequisite for this optimization.
...
- [x] Run the full correctness, golden, type, compilation, deployment-sanity,
  and performance suites required by Section 4.
  Evidence: 328 unit tests, normal and extended enforced desktop budgets,
  golden, mypy, compilation, deployment sanity, stress, soak, and isolated
  black-box acceptance all pass.
...
- [x] Retain non-hexadecimal legacy chain IDs as a separate fallback workload;
  ...
  result.
- [x] Include `stage_seconds`, `export_seconds`, outbox transactions and
  connections, and command duration by purpose in compact reconcile reports.
  Evidence: compact reconcile reports now retain stage, export, lock-contention,
  and command-purpose telemetry; commit `ae7f5f6`.
...
- [x] Measure at least three independent apply samples, each using fresh
  Taskdata; a previously applied fixture is not a valid repeat.
  Evidence: `budget.desktop.per7.json` contains four fresh production-shaped
  apply samples at 1, 8, 32, and 200 candidates.
...
- [x] Prove that every safe plan is durable before the first Taskwarrior
...
- [x] Preserve immutable-plan compatibility, configuration and schedule
...
- [x] Cover interruption after bulk staging and after bulk exact claiming; all
  durable rows must remain recoverable without duplicate mutation.
- [x] Repeat queue-versus-reconcile and reconcile-versus-reconcile process
  races and prove that exactly one owner can claim each intent.
  Evidence: `tests/test_lifecycle_failure_injection.py` starts independent
  Python processes for three queue/exact races and three exact/exact races;
  the complete staged wave is owned once with no duplicate claims.
...
- [x] Extract or share the existing exact-claim decision logic, including
...
- [x] Reuse the repository's existing bulk-connection/CAS pattern; keep the
...
- [x] If a stage or claim result blocks the wave, return its typed evidence and
...
- [x] Retain the authoritative child-slot occupancy check used during wave
  planning.
- [x] Retain deterministic child-UUID collision checks before import.
- [x] Retain a fresh parent recurrence/state guard before mutation.
- [x] Retain the per-parent guarded `task modify`; each parent receives a
  different `nextLink` and cannot safely share one unguarded update.
- [x] Retain phase-wide authoritative child verification after batch import
  and parent verification after guarded linking.
- [x] Do not replace guarded parent modification with Taskwarrior import, omit
  postconditions, weaken mutation-epoch checks, or parallelize mutations to
  meet the performance target.
- [x] Preserve strict JSON output, Unicode output, diagnostics routing,
  idempotence, and recovery reporting unchanged.
  Evidence: authoritative prefetch/postverification, collision, parent-guard,
  mutation-contract, and crash-resume tests pass in the golden and focused
  suites.

#### Completion Gate For This Subsection

- [x] Demonstrate that a 32-plan wave uses no more than two SQLite transactions
  for stage and exact claim, instead of the current 64.
  Evidence: `test_execute_wave_claims_only_its_staged_intents` instruments the
  repository and confirms exactly two durable transactions for the stage and
  exact-claim phases; the claimed drain is presentation-only in this contract.
- [x] Demonstrate at least a 15% reduction in the corrected production-shaped
  ...
  Evidence: `budget.desktop.per7.json` measured 1.672 s for 32 candidates
  versus 2.721 s in the recorded baseline (38.6% faster); 200 candidates
  measured 13.516 s versus 19.904 s (32.1% faster).
- [x] Confirm no increase in Taskwarrior mutation calls, verification calls,
  exported rows, retry attempts, peak memory, or failure-path latency budgets.
  Evidence: the fresh apply reports retain per-purpose call counts, bounded
  exports/attempts, and no failed checks; the 32-candidate path uses one batch
  child import plus guarded parent links with no retry or budget overflow.
- [x] Confirm identical plans, applied records, Taskwarrior state, and durable
  outbox state for 1, 8, 32, and 200 candidates.
  Evidence: `budget.desktop.per7.json` reports successful applied runs at all
  four scales with bounded calls and no failed checks.
- [x] Run focused outbox, lifecycle application, reconcile, interruption,
  poison-row, stale-lease, configuration-drift, and process-race tests.
  Evidence: focused failure-injection suite passes 14 tests, including the
  repeated subprocess process-race contracts.
- [x] Run full unit, normal and shuffled golden, type, compilation,
  deployment-sanity, stress, and enforced performance suites.
  Evidence: unit 328 passed; golden normal and shuffled 989/989; full mypy,
  compilation, deployment sanity, and enforced CI stress all pass. Enforced
  desktop performance is green; Termux variance is recorded above.
- [x] Run the corrected workload on the supported Termux reference device and
  retain its existing absolute reliability budget.
  Evidence: `benchmarks/optimization-v1/termux-device1-opt7.json` completed
  all queue/reconcile workflows successfully. Cold-core module count and the
  marginal seasonal timing remain accepted pre-existing variance; no budget
  was raised and no reliability check failed.
- [x] Revert or revise the optimization if its end-to-end result remains within
  the accepted noise floor; do not remove safety work or raise a budget to
  manufacture acceptance.
  Evidence: desktop and Termux reconcile paths exceed the accepted improvement
  target; unrelated cold-import variance was retained rather than masked.
- [x] Commit benchmark repair separately from production implementation, and
  keep unrelated cleanup out of both commits.
  Production outbox changes and the independent benchmark timeout repair
  (`79bcd56`) are separate commits.

## 4. Final Offline Acceptance

- [x] Run full unit discovery; zero failures and errors.
  Evidence: canonical discovery completed 328 tests with zero failures/errors.
- [x] Run the normal golden suite; all tests pass.
  Evidence: `python3 dev_tools/nautical_golden_tests.py` completed 989/989.
- [x] Run the shuffled golden suite with seed `20260811`; all tests pass with no
  order-dependent leakage.
  Evidence: shuffled run completed 989/989.
- [x] Run configured mypy and the strict full-package/operator checks used by
  `.github/workflows/type-check.yml`; all report zero errors.
- [x] Run source compilation, `git diff --check`, and deployment sanity.
  Evidence: full-package mypy passed for 231 files; source compilation,
  diff checks, and `nautical_deploy_sanity.py --json` passed.
- [x] Run normal and extended enforced performance budgets.
  Evidence: both desktop runs completed with `enforced: true`, `ok: true`, and
  `failed_checks: []`.
- [x] Run the enforced CI stress campaign.
  Evidence: `nautical_stress_campaign.py --profile ci --json --enforce`
  passed the mixed-recurrence stage (8 cycles, 9.436 s).
- [x] Run an enforced 10-minute release-candidate soak:

```bash
python3 dev_tools/nautical_soak_test.py --minutes 10 --json --enforce
```

  Evidence: `/tmp/soak.wave.json` passed 604.97 s, 125 cycles, 2,500 adds,
  1,500 completions, zero failures, healthy queue, and zero dead letters.

- [x] Exercise CP and anchor completion, immediate expiration, queue
  ...
  Evidence: `dev_tools/nautical_black_box_test.py --json` passed with
  `ok: true`, including recovery, multi-hop, duplicate-guard, and operator
  cutover scenarios.
- [x] Verify strict add/modify hook JSON stdout and Unicode preservation.
  Evidence: hook workflow and operator process contract tests pass; the
  isolated black-box run completed successfully.
- [x] Verify malformed hook input cannot crash the hook or corrupt stdout.
  Evidence: hook contract and golden malformed-input coverage pass.
- [x] Verify read-only commands cannot reach mutation owners.
  Evidence: black-box dispatcher and read-only operator contract checks pass.
- [x] Verify repeated effectful operations converge idempotently after injected
  interruption at each affected durable boundary.
  Evidence: lifecycle failure-injection suite and black-box recovery paths
  pass without duplicate children or unrecoverable intents.
- [x] Compare the final candidate with the recorded green baseline.
  Evidence: desktop normal/extended enforced budgets and the recorded Termux
  device-1 report remain within the accepted reliability budgets and improve
  the reconcile target.
- [x] Explain every material metric delta, including improvements and
  regressions.
  Evidence: accepted Termux cold-import/seasonal variance is documented; no
  reliability budget was raised.
- [x] Confirm no budget was raised solely to accept the candidate.

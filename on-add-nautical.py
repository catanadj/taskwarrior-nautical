#!/usr/bin/env python3
"""Thin on-add launcher with a plain-task fast path."""
...
def _diagnose(message: str) -> None:
    if os.environ.get("NAUTICAL_DIAG") != "1":
        return
    try:
        sys.stderr.write(f"[nautical] on-add {message}\n")
    except Exception:
        pass
...
def main() -> int:
    ...
    protocol, _protocol_path, protocol_error = hook_bootstrap.load_core_helper_module(
        _CORE_BASE,
        "hook_protocol.py",
        "_nautical_hook_protocol_add_wrapper",
    )
    probe = None
    if protocol is not None:
        try:

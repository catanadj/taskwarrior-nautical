...
def preflight_context(host: Any, new: TaskPayload, now_utc: datetime, repository):
    ...
    services = runtime.build_preflight_services(
        ...
        completion_chain_snapshot=lambda chain_id, base_no, next_no: chain_snapshot(
            host, chain_id, base_no, next_no, repository
        ),
        completion_existing_next_or_fail=lambda task, next_no, snapshot: existing_next_or_fail(
            host, task, next_no, snapshot, repository
        ),
    )

    ...
    *,
    ...
    if typ == "m":
        if random_match:
            ...
            bad = [key for key in active if key not in ("t", "time_window", "time_schedule", "time_random", "moon", "bd", "wd", "time_offset_minutes")]
            if bad:
    ...
    if typ == "moon":
        ...
        bad = [key for key in active_mod_keys(mods) if key not in ("t", "time_window", "time_schedule", "time_random", "time_offset_minutes")]
        if bad:

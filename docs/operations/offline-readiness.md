...
Runtime packages use lower bounds and compatibility-tested major-version caps
in the checked-in `requirements.txt` files. If a target needs a different set,
qualify that set with the compatibility tests and build a new kit; do not
install unconstrained upgrades during offline recovery.

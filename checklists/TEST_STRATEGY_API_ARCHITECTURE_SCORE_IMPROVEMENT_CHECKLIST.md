...
per-domain inventory in the local-only `design/GOLDEN_REGISTRY_INVENTORY.md`.
Historical
...
Scheduler owner cutover was completed in commits `805f31f`, `b30f96f`,
`c05d67c`, and `d00c608`. The focused scheduler contract, cross-path, and
runtime suite passes 61/61; the full suite passes 1,221 tests (3 skipped).
...
the direct chain-integrity engine boundary test now also covers durable outbox
failure before mutation. Untracked local audit artifacts remain intentionally
excluded.
